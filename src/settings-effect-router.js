...
function createSettingsEffectRouter(options = {}) {
  ...
  function handleSettingsChange({ changes } = {}) {
    ...
    if ("showTray" in changes) {
      safeCall(
        logWarn,
        "Roary: tray toggle failed:",
        changes.showTray ? createTray : destroyTray
      );
    }
    if ("showDock" in changes) {
      safeCall(logWarn, "Roary: applyDockVisibility failed:", applyDockVisibility);
    }
    ...
    if ("keepAwakeWhileWorking" in changes) {
      safeCall(logWarn, "Roary: reconcilePowerSaveBlocker failed:", reconcilePowerSaveBlocker);
    }
    if ("lang" in changes) {
      safeCall(logWarn, "Roary: dashboard lang broadcast failed:", sendDashboardI18n);
      safeCall(logWarn, "Roary: session HUD lang broadcast failed:", sendSessionHudI18n);
    }
    if ("sessionAliases" in changes) {
      safeCall(
        logWarn,
        "Roary: session alias snapshot broadcast failed:",
        emitSessionSnapshot,
        { force: true }
      );
    }
    ...
    if ("hideBubbles" in changes || "permissionBubblesEnabled" in changes) {
      safeCall(logWarn, "Roary: syncPermissionShortcuts failed:", syncPermissionShortcuts);
    }
    if (
      ...
      safeCall(
        logWarn,
        "Roary: dismiss interactive bubbles failed:",
    ...
    if (
      ...
      try {
        ...
        warn(logWarn, "Roary: clear notification bubbles failed:", err);
      }
    } else if (
      ...
      safeCall(
        logWarn,
        "Roary: refresh notification bubble timers failed:",
    ...
    if (
      ...
      safeCall(logWarn, "Roary: hide update bubble failed:", hideUpdateBubbleForPolicy);
    } else if (
      ...
      safeCall(
        logWarn,
        "Roary: refresh update bubble timer failed:",
    ...
    if ("permissionBubbleAutoCloseSeconds" in changes) {
      safeCall(
        logWarn,
        "Roary: refresh permission bubble timer failed:",
        refreshPermissionAutoCloseForPolicy
      );
    }
    if ("bubbleFollowPet" in changes) {
      safeCall(logWarn, "Roary: repositionFloatingBubbles failed:", repositionFloatingBubbles);
    }
    if ("textScale" in changes || "textScaleByDisplay" in changes) {
      ...
      safeCall(logWarn, "Roary: applyTextScale failed:", applyTextScale);
    }
    if ("sessionHudPinned" in changes) {
      ...
      try {
        ...
        warn(logWarn, "Roary: session HUD pinned change failed:", err);
      }
    }
    if (
      ...
      try {
        ...
        warn(logWarn, "Roary: session HUD setting sync failed:", err);
      }
    }
    if ("sessionHudCleanupDetached" in changes && changes.sessionHudCleanupDetached === true) {
      try {
        ...
        warn(logWarn, "Roary: detached session cleanup sweep failed:", err);
      }
    } else if ("sessionHudCleanupDetached" in changes) {
      safeCall(
        logWarn,
        "Roary: detached session cleanup snapshot refresh failed:",
        emitSessionSnapshot,
        { force: true }
      );
    }
    if (
      ...
      try {
        ...
        warn(logWarn, "Roary: stale cleanup config refresh failed:", err);
      }
    }
    if ("allowEdgePinning" in changes) {
      safeCall(
        logWarn,
        "Roary: allowEdgePinning re-clamp failed:",
        reclampPetAfterEdgePinningChange
      );
    }
    if ("disableMiniMode" in changes && changes.disableMiniMode && getMiniMode()) {
      safeCall(logWarn, "Roary: disableMiniMode exit failed:", exitMiniMode);
    }
    ...
    for (const key of Object.keys(changes)) {
      if (MENU_AFFECTING_KEYS.has(key)) {
        safeCall(logWarn, "Roary: rebuildAllMenus failed:", rebuildAllMenus);
        break;
      }
    }
    ...
    try {
      ...
      warn(logWarn, "Roary: settings-changed broadcast failed:", err);
    }
  }
