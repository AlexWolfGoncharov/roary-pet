...
function createClaudeSettingsWatcher(ctx = {}) {
  ...
  function start() {
    ...
    try {
      ...
      console.warn("Roary: could not seed settings baseline:", err.message);
    }
    try {
          ...
          try {
            ...
            if (settingsNeedClaudeHookResync(raw, expectedPermissionUrl)) {
              ...
              if (isSuspiciousShrink(lastTrustedSnapshot, currentSnapshot, suspiciousShrinkRatio, suspiciousKeyLossThreshold)) {
                console.warn("Roary: settings.json shrank suspiciously — skipping auto-resync to preserve third-party hooks");
                if (typeof ctx.notifySuspiciousShrink === "function") {
                  ctx.notifySuspiciousShrink(lastTrustedSnapshot, currentSnapshot);
                }
                return;
              }
              console.log("Roary: hooks missing from settings.json — re-registering");
      ...
      if (settingsWatcher && typeof settingsWatcher.on === "function") settingsWatcher.on("error", (err) => {
        console.warn("Roary: settings watcher error:", err.message);
      });
      return true;
    } catch (err) {
      console.warn("Roary: failed to watch settings directory:", err.message);
      settingsWatcher = null;
      return false;
    }
  }
