...
const SCHEMA = {
  ...
  // Pure data prefs
  lang: { type: "string", default: "en", enum: ["en", "ru", "uk"] },
  ...
  // Theme
  theme: { type: "string", default: "roary" },
...
function load(prefsPath) {
  ...
  try {
    ...
    try {
      ...
      console.warn(`Roary: prefs file unreadable, backed up to ${bak}:`, err.message);
    } catch (bakErr) {
      console.warn("Roary: prefs file unreadable and backup failed:", err.message, bakErr.message);
    }
    return { snapshot: getDefaults(), locked: false };
  }
  ...
  if (incomingVersion > CURRENT_VERSION) {
    console.warn(
      `Roary: prefs file version ${incomingVersion} is newer than supported (${CURRENT_VERSION}). ` +
