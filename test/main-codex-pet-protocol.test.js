...
test("main wires roary:// protocol dispatch through the Codex Pet importer", () => {
...
test("package metadata registers the clawd protocol and exposes dev registration", () => {
  ...
  assert.ok(protocols.some((entry) => Array.isArray(entry.schemes) && entry.schemes.includes("roary")));
