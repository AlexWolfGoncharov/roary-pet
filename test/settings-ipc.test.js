...
test("settings IPC imports Clawd user theme zip packages", async () => {
  ...
  try {
    ...
    assert.deepStrictEqual(dialogOptions.filters, [{ name: "Roary theme zip", extensions: ["zip"] }]);
    assert.strictEqual(
...
test("settings IPC serves agent/about/update/external and remove-theme dialog helpers", async () => {
  ...
  try {
    ...
    assert.deepStrictEqual(await ipcMain.invoke("settings:get-about-info"), {
      version: "1.2.3",
      repoUrl: "https://github.com/AlexWolfGoncharov/roary-pet",
      license: "AGPL-3.0",
      copyright: "\u00a9 2026 Fintech Farm (fork of clawd-on-desk \u00a9 Ruller_Lulu)",
      authorName: "Fintech Farm",
      authorUrl: "https://github.com/AlexWolfGoncharov/roary-pet",
